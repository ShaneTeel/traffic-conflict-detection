...
class TimeToCollision:
    ...
    def calculate_instant_ttc(self, traj_A:TrajAnalyzer, traj_B:TrajAnalyzer, time:float):
        ...
        if dot_product > 0:
            logger.debug("Objects are moving apart; no collision.")
            return dummy
        
        logger.debug(f"Objects at {time} are moving closer together; determining potential for collision.")
        ...
        if t < 0:
            logger.debug("Closest approach was in the past. No future collision detected.")
            return dummy
        
        logger.debug(f"Objects will be closest in {t:.2f} seconds.")   
    ...
    def analyze_all_conflicts(self, analyzers:dict[TrajAnalyzer], start:float=None, end:float=None, step:float=0.1):
        '''
        ...
        for i in range(len(analyzer_lst) - 1):
            traj_A = analyzer_lst[i]
            for traj_B in analyzer_lst[i+1:]:
    ...
    def _get_overlap_period(self, traj_A: TrajAnalyzer, traj_B: TrajAnalyzer):
        ...
        logger.info(f"Analyzing tracks {traj_A.track_id} and {traj_B.track_id} from {start:.2f} to {end:.2f} seconds.")
        return start, end